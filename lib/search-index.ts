...
Guest (незарег) — только смотрит, не может писать и тратить кредиты. Member (базовый) — может писать, покупать кредиты, участвовать в шоу. Levels (уровни) — чем выше уровень, тем больше юзер потратил, активно взаимодействуй с высокими уровнями.
  ...
  },
  {
    ...
    section: "Инструменты",
    keywords: [
      "simulator", "bongacams", "practice", "интерфейс", "interface",
      "model", "stream", "chat", "settings", "панель управления",
      "чат", "настройки", "практика", "обучение", "симулятор"
    ],
    content:
      "Интерактивный симулятор интерфейса модели BongaCams. Позволяет практиковаться с панелью управления стримом, чатом и настройками. Симуляция включает: управление трансляцией (старт/стоп, приватный режим), настройки камеры и микрофона, работу с целями (goals), автоматическую генерацию сообщений в чате, просмотр списка зрителей, и настройки профиля. Все данные являются имитацией для обучения.",
  },
  {
